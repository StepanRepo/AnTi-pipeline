...
Int_profile::Int_profile(Raw_profile& raw, vector<double>* mask) : session_info()
{
	...
	if (!mask)
	{
		mask = new vector<double>(channels);
...
double Int_profile::get_reper_point (Etalon_profile& etalon)
{
	...
	ccf = discrete_ccf(profile, etalon.profile);
	...
	{
		vector<double> ccf_moved = ccf;
		move_continous(ccf_moved, session_info.get_PSR_PERIOD()*1e3/session_info.get_TAU()) ;

		for (size_t i = 0; i < ccf.size(); ++i)
			ccf.at(i) += ccf_moved.at(i);

		max_ = max(ccf);
		max_pos_ = max_pos(ccf);
	...
	for (int i = -2; i < 3; i++)
		near_max.push_back(ccf.at(max_pos_ + i)/max_);
